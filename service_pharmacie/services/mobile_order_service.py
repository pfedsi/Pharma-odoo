...
class MobileOrderService(models.AbstractModel):
    ...
    def attach_ticket(self, mobile_order, ticket):
        mobile_order.sudo().write({"ticket_id": ticket.id, "state": "arrived"})
    ...
    def confirm_to_pos_order(self, mobile_order):
        ...
        for line in mobile_order.line_ids:
            # ✅ product_variant_ids[:1] fonctionne quelle que soit le nb de variantes
            product_variant = line.product_tmpl_id.product_variant_ids[:1]
            if not product_variant:
                _logger.warning(
                    "Ligne %s ignorée : pas de variante pour product_tmpl_id=%s",
                    line.id, line.product_tmpl_id.id,
                )
                continue
        ...
        if not line_commands:
            raise ValidationError(_("Aucune variante produit valide."))
        ...
        mobile_order.sudo().write({"pos_order_id": pos_order.id, "state": "confirmed"})
    ...
    def export_order_payload(self, order):
        ...
        for l in order.line_ids:
            product_tmpl = l.product_tmpl_id
            # ✅ product_variant_ids[:1] au lieu de product_variant_id
            # product_variant_id retourne vide si le template a plusieurs variantes
            product_variant = product_tmpl.product_variant_ids[:1] if product_tmpl else False

            if not product_variant:
                _logger.warning(
                    "[export_order_payload] Ligne %s : aucune variante pour template %s — ignorée du payload POS",
                    l.id,
                    product_tmpl.id if product_tmpl else "?",
                )

            lines_data.append({
                "id":                    l.id,
                # ✅ product.product.id — utilisé par getProductFromPos() côté JS
                "product_id":            product_variant.id if product_variant else False,
                # product.template.id — utile pour affichage mobile / image
    ...
    def export_order_payload_for_pos(self, order):
        """
        Version allégée spécifiquement pour l'injection dans le POS via l'event
        prescription-scanned. Ne renvoie que ce dont le JS a besoin.
        """
        result = []
        for l in order.line_ids:
            product_tmpl = l.product_tmpl_id
            ...
            if not product_variant:
                _logger.warning(
                    "[export_order_payload_for_pos] Ligne %s sans variante — skippée", l.id
                )
                continue

            result.append({
                "product_id":      product_variant.id,    # product.product.id ✅
                "product_tmpl_id": product_tmpl.id,       # product.template.id
                "name":            l.name,
                "quantity":        l.quantity,
                "price_unit":      l.price_unit,
                "source_type":     l.source_type,
